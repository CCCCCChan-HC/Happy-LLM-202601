...
- [QA] Q: 点积的几何意义？相似大于0，不相似小于0？）
    > A: 点积\( a \cdot b = |a||b|\cos\theta \)（\( \theta \)是两向量的夹角）：
        - 本质是“一个向量在另一个向量方向上的投影长度乘以另一个向量的模长”；
        - 当两向量方向一致（\( \theta=0^\circ \)），\( \cos\theta=1 \)，点积最大；
        - 当两向量垂直（\( \theta=90^\circ \)），\( \cos\theta=0 \)，点积为0；
        - 当两向量方向相反（\( \theta=180^\circ \)），\( \cos\theta=-1 \)，点积最小。
        **相似性判断**
        - 点积的正负**不直接等同于“相似/不相似”**，而是反映“方向是否一致”：
        - 点积>0：两向量夹角<90°，方向大致相同（语义倾向一致，如“猫”和“狗”都是动物）；
        - 点积=0：两向量垂直，无直接关联（如“猫”和“桌子”）；
        - 点积<0：两向量夹角>90°，方向相反（语义倾向相反，如“喜欢”和“讨厌”）。
        - 注意：相似性需结合向量模长（或归一化后）判断——归一化后（\( |a|=|b|=1 \)），点积直接等于\( \cos\theta \)，取值范围[-1,1]，此时“点积越大，相似性越高”。
